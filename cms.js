...
function render(el, items, tpl) {
    if (!el) return;
    el.innerHTML = items.length ? items.map(tpl).join("") : "<p>Geen data</p>";
}
...
window.deleteHighlight = id => remove(ref(db,"highlights/"+id));
...
onValue(ref(db,"club100"), snap=>{
    ...
    render(clubList, items, p=>`
        <div class="tile">
            ...
            <div>${(p.name||"").split(" ").slice(1).join(" ")}</div>
            <button onclick="deleteClub('${p.id}')">🗑</button>
        </div>
    `);
});

window.deleteClub = id => remove(ref(db,"club100/"+id));
...
window.deleteSponsor = id => remove(ref(db,"sponsors/"+id));
...
window.deleteOmeJan = id => remove(ref(db,"omejan/"+id));
